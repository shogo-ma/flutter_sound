...
- (bool)initializeFlautoRecorder:(t_AUDIO_FOCUS)focus
  ...
  BOOL r = [self setAudioFocus:focus
                      category:category
                          mode:mode
...
- (bool)startRecorderCodec:(t_CODEC)codec
  ...
  if (tabSessionPort[audioSource] != 0) {
    for (AVAudioSessionPortDescription *portDescr in availableInputs) {
      ...
      if ([port isEqual:tabSessionPort[audioSource]]) {
        ...
        for (AVAudioSessionSourceDescription *dataSource in port
                 .inputDataSources) {
          NSLog("dataSource :%@", dataSource.dataSourceName);
        }
        break;
        // found = true;
      }
  ...
  NSMutableDictionary *audioSettings = [NSMutableDictionary
      dictionaryWithObjectsAndKeys:[NSNumber numberWithLong:sampleRate],
                                   AVSampleRateKey,
                                   [NSNumber numberWithInt:formats[codec]],
                                   AVFormatIDKey,
                                   [NSNumber numberWithInt:numChannels],
...
- (void)startRecorderTimer {
  ...
  if (subscriptionDuration > 0) {
    recorderTimer = [NSTimer
        scheduledTimerWithTimeInterval:subscriptionDuration
                                target:self
...
// post fix with _FlutterSound to avoid conflicts with common libs including
// path_provider
...
- (void)updateRecorderProgress:(NSTimer *)atimer {
  ...
  [m_callBack updateRecorderProgressDbPeakLevel:normalizedPeakLevel
                                       duration:duration];
}
